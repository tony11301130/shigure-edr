...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title="Shigure API", version="0.1.0")
...
<head>
  ...
  <title>Shigure // Investigation Console</title>
  <style>
  ...
  <main class="shell">
    <aside class="pane queue">
      ...
      <div class="filters" id="alertFilters">
        <div class="saved-searches" aria-label="Saved alert searches"><button class="saved active" id="presetAll" onclick="setAlertPreset('all')">All</button><button class="saved" id="presetHigh" onclick="setAlertPreset('high')">High</button><button class="saved" id="presetEvidence" onclick="setAlertPreset('evidence')">Evidence</button><button class="saved" id="presetClear" onclick="clearAlertFilters()">Clear</button></div>
        <div class="filter-grid">
          <select id="filterSeverity" aria-label="Alert severity filter" onchange="applyAlertFilters()"><option value="">Severity</option><option value="critical">Critical</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option><option value="info">Info</option></select>
          <input id="filterHost" placeholder="Endpoint / host" aria-label="Alert host filter" oninput="debouncedAlertFilters()">
          <input id="filterTitle" placeholder="Rule / title" aria-label="Alert rule filter" oninput="debouncedAlertFilters()">
          <input id="filterMitre" placeholder="MITRE / artifact / IOC" aria-label="Alert MITRE artifact indicator filter" oninput="debouncedAlertFilters()">
        </div>
      </div>
    ...
    <aside class="pane right">
      ...
      <div class="pane-body">
        ...
        <section class="section"><h3>Evidence ribbon</h3><div id="evidenceRibbon" class="evidence-ribbon"></div></section>
...
async function loadAll(){try{setText('liveText','refreshing…');const t=tenant();const alertQuery=alertFilterQuery(t);const [summary,agents,alerts,cases,tasks,hunts,evidence,events,catalog]=await Promise.all([api(`/api/v1/admin/summary?tenant_id=${t}`),api(`/api/v1/admin/agents?tenant_id=${t}`),api(`/api/v1/admin/alerts?${alertQuery}`),api(`/api/v1/admin/cases?tenant_id=${t}&limit=25`),api(`/api/v1/admin/tasks?tenant_id=${t}&limit=50`),api(`/api/v1/admin/hunts?tenant_id=${t}&limit=25`),api(`/api/v1/admin/raw-evidence/list?tenant_id=${t}&limit=50`),api(`/api/v1/admin/events?tenant_id=${t}&limit=150`),api('/api/v1/admin/task-catalog')]);STATE.summary=summary;STATE.agents=agents.agents||[];STATE.alerts=alerts.alerts||alerts||[];STATE.cases=cases.cases||cases||[];STATE.tasks=tasks.tasks||tasks||[];STATE.hunts=hunts.hunts||hunts||[];STATE.evidence=evidence.evidence||evidence||[];STATE.events=events.events||events||[];STATE.taskCatalog=catalog.tasks||[];if(STATE.selectedAlert&&!STATE.alerts.find(a=>a.alert_id===STATE.selectedAlert.alert_id))STATE.selectedAlert=null;if(!STATE.selectedAlert&&STATE.alerts[0])selectAlert(STATE.alerts[0].alert_id,false);renderAll();setText('liveText','updated '+new Date().toLocaleTimeString())}catch(e){setText('liveText','load failed');$('queueBody').innerHTML=empty(e.message)}}
...
function switchQueue(q){STATE.queue=q;$('tabAlerts').classList.toggle('active',q==='alerts');$('tabEndpoints').classList.toggle('active',q==='endpoints');$('alertFilters').style.display=q==='alerts'?'grid':'none';renderQueue()}function alertFilterQuery(t){const p=new URLSearchParams({tenant_id:decodeURIComponent(t),limit:'75'});const sev=$('filterSeverity').value,host=$('filterHost').value.trim(),title=$('filterTitle').value.trim(),wide=$('filterMitre').value.trim();if(sev)p.set('severity',sev);if(host)p.set('host',host);if(title)p.set('title',title);if(wide)p.set('indicator',wide);if($('presetEvidence').classList.contains('active'))p.set('has_evidence','true');return p.toString()}function debouncedAlertFilters(){clearTimeout(FILTER_TIMER);FILTER_TIMER=setTimeout(applyAlertFilters,250)}function applyAlertFilters(){['presetAll','presetHigh','presetEvidence'].forEach(id=>$(id).classList.remove('active'));loadAll()}function resetAlertFilterInputs(){$('filterSeverity').value='';$('filterHost').value='';$('filterTitle').value='';$('filterMitre').value='';['presetHigh','presetEvidence'].forEach(id=>$(id).classList.remove('active'));$('presetAll').classList.add('active')}function clearAlertFilters(){resetAlertFilterInputs();loadAll()}function setAlertPreset(p){resetAlertFilterInputs();if(p==='high'){$('presetAll').classList.remove('active');$('presetHigh').classList.add('active');$('filterSeverity').value='high'}if(p==='evidence'){$('presetAll').classList.remove('active');$('presetEvidence').classList.add('active')}if(p==='all')$('presetAll').classList.add('active');loadAll()}function renderQueue(){if(STATE.queue==='endpoints')return renderEndpointQueue();const rows=STATE.alerts;if(!rows.length){$('queueBody').innerHTML=empty('No alerts match the current facets. Clear filters or widen the time and rule search.');return}$('queueBody').innerHTML=rows.map(a=>{const active=STATE.selectedAlert&&STATE.selectedAlert.alert_id===a.alert_id;const sev=clsSeverity(a.severity);const evidence=a.raw_ref||a.raw_hash?'evidence':'no evidence';return `<div class="alert-row ${active?'active':''}" onclick="selectAlert('${esc(a.alert_id)}')"><div class="alert-main"><div class="sevbar ${sev}"></div><div class="titleline"><b>${esc(a.title)}</b><div class="meta"><span class="sev-${sev}">${esc(a.severity)}</span><span>${esc(a.host||'unknown host')}</span><span>${esc((a.mitre||[])[0]||'no MITRE')}</span></div></div><div class="time">${esc(fmtTime(a.timestamp||a.created_at))}</div></div><div class="meta2">${esc(a.process_name||'no process')} · ${esc(a.source||'unknown source')} · ${esc(evidence)} · ${esc(short(a.description||a.alert_id,96))}</div></div>`}).join('')}
...
function renderContext(){const a=STATE.selectedAlert,ep=STATE.selectedEndpoint;let detail=[];if(a){detail=[['Severity',a.severity],['Host',a.host],['Process',a.process_name],['MITRE',(a.mitre||[]).join(', ')],['Time',fmtTime(a.timestamp||a.created_at)],['Alert ID',a.alert_id]]}else if(ep){detail=[['Host',ep.host],['Status',ep.status],['IP',ep.ip_address],['OS',ep.os],['Version',ep.agent_version],['Agent ID',ep.agent_id]]}else detail=[['Selection','none'],['Hint','Pick an alert']];$('entityDetail').innerHTML=detail.map(([k,v])=>`<div>${esc(k)}</div><div title="${esc(v)}">${esc(v||'—')}</div>`).join('');renderEvidenceRibbon();renderRecommendedActions()}function renderEvidenceRibbon(){const a=STATE.selectedAlert,ep=STATE.selectedEndpoint;const agent=ep||(a&&findAgentByHost(a.host))||{};const latestTask=(STATE.tasks||[]).find(t=>agent.agent_id&&t.agent_id===agent.agent_id&&t.raw_ref)||(STATE.tasks||[]).find(t=>t.raw_ref)||{};const cells=[['normalized event',a?(a.raw&&a.raw.event_id)||short(a.alert_id,28):'select alert'],['raw evidence',a&&a.raw_ref?a.raw_ref:latestTask.raw_ref||'not attached'],['hash',a&&a.raw_hash?a.raw_hash:latestTask.raw_hash||'pending'],['collector',a?(a.source||'internal'):'—'],['endpoint',a&&a.host?a.host:agent.host||'—'],['task link',latestTask.task_id||'no task evidence']];$('evidenceRibbon').innerHTML=cells.map(([k,v])=>`<div class="ribbon-cell"><span>${esc(k)}</span><b title="${esc(v)}">${esc(v)}</b></div>`).join('')}
